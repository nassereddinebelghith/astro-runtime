# Custom Keycloak Airflow OIDC provider
# UMA is disabled in this AuthManager
...
class OidcKeycloakAuthManager(KeycloakAuthManager):
    ...
    def refresh_user(
    ...
    ) -> Optional[KeycloakAuthManagerUser]:
        ...
        try:
            ...
            if "invalid_grant" in msg or "token is not active" in msg:
                log.info("Refresh token expired -> forcing re-login")
            else:
                log.warning("Unexpected Keycloak refresh error: %s", exc)

            if has_request_context():
                session.clear()
                session["kc_refresh_failed"] = True
        ...
        if has_request_context():
            session.pop("kc_refresh_failed", None)
