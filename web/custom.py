...
class OidcKeycloakAuthManager(KeycloakAuthManager):
    ...
    def _safe_clear_session(self):
        """
        Safely clear Flask session, handling 'Working outside of request context' error.
        """
        try:
            from flask import session, has_request_context
            
            # Only clear if we're actually in a request context
            if has_request_context():
                session.clear()
                log.debug("Session cleared successfully")
            else:
                log.debug("Not in request context, skipping session clear")
    ...
    def _is_authorized(
        ...
        *,
        ...
        # ==========================================================================
        # MODIFICATION 2 : Catch KeycloakPostError pour invalid_grant
        # ==========================================================================
        if hasattr(user, 'access_token') and user.access_token:
            if self._token_expired(user.access_token):
                # CHANGEMENT : Ajout du try/except pour catch invalid_grant
                try:
                    ...
                    # CATCH l'erreur "invalid_grant" qui causait le 500
                    error_str = str(e)
                    if "invalid_grant" in error_str or "Token is not active" in error_str:
                        log.warning(f"Refresh token expired (invalid_grant), clearing session")
                        self._safe_clear_session()
                        return False
                    else:
