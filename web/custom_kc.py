...
class OidcKeycloakAuthManager(KeycloakAuthManager):
    ...
    def is_logged_in(self) -> bool:
        """
        Check if user is logged in with intelligent handling:
        - No user → True (allow public access to landing page)
        - User with valid token → True
        - User with expired token → False (trigger OAuth)
        """
        try:
            user = self.get_user()
            
            if not user:
                # No user at all → Allow public access (landing page)
                return True
    ...
    def _is_authorized(
        ...
        *,
        ...
        # Check token expiration and try refresh
        if hasattr(user, 'access_token') and user.access_token:
            if self._token_expired(user.access_token):
                log.info("Token expired in _is_authorized, attempting refresh")
                ...
                if refreshed:
                    ...
                    # Refresh failed - need to trigger OAuth by raising exception
                    log.warning("Token refresh failed, forcing re-authentication")
                    from airflow.exceptions import AirflowException
                    raise AirflowException("Authentication expired - please log in again")
